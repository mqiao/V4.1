...
public class RestaurantPanel extends JPanel {
    //create animation
    static int gridX = 20;//was20
    static int gridY = 15;//was15
	...
	if(type.equals("Customers")){
	    CustomerAgent c = new CustomerAgent(name, gui, restaurant,getMenu());
	    c.setHost(host);
    ...
    public void addCustomer(String name, double money, String order, boolean ifWait)
    {
    	CustomerAgent c = new CustomerAgent(name,gui,restaurant,money,order,ifWait);
    	c.setHost(host);
    	c.setCashier(cashier);
    	customers.add(c);
	    c.startThread(); //Customer is fsm.
	    c.setHungry();
	    customerPanel.createNewCustomer(name);
    }
    
    public void addItemToMenu(String name, double price)
    {
    	host.addItemToMenu(name, price);
    }
    public void updateItemToMenu(String name, double price)
    {
    	host.modifyMenu(name,price);
    }
    public void updateItemToCook(String name, int amount, int max, int threshold, int prepTime, ArrayList<Boolean> MKT)
    {
    	cook.updateFoodInfo(name, amount, max, threshold, prepTime,MKT,getMenu());   
    
    }
    public int getFoodAmount(String name)
    {
    	return(cook.getAmount(name));
    }
    public int getFoodMax(String name)
    {
    	return(cook.getMax(name));

    }
    public int getFoodThreshold(String name)
    {
    	return(cook.getThreshold(name));
    }
    public int getPrepTime(String name)
    {
    	return(cook.getPrepTime(name));
    }
    public boolean getMarketAvailability(int i,String foodName)
    {
    	return(cook.getMarketAvailability(i,foodName));
    }
