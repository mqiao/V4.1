...
{   
    ...
    protected int getCashierX()
    {
    	return cashierX;
    }
    protected int getCashierY()
    {
    	return cashierY;
    }
...
//            foodName = foodName.substring(0, 3);
        ...
        buttons[oldx-1][oldy-1].setText(defaultText);
        buttons[newx-1][newy-1].setForeground(color);
        buttons[newx-1][newy-1].setText(waiterName + "BL");
        try
        {
            Thread.sleep(animDelay);
        }
    ...
    protected void placeCashier(int x, int y, Color color, String name)
    {
    	cashierX=x;
    	cashierY=y;
        if (name.length() > 2)
    ...
    public void placeBill(int x, int y, Color color, String name)
    {
        buttons[x-1][y-1].setForeground(color);
        buttons[x-1][y-1].setText(name);
        try
    ...
    protected void removeBill(int x, int y)
    {
        buttons[x-1][y-1].setForeground(buttons[x-1][y-1].getBackground());
        buttons[x-1][y-1].setText(defaultText);
        try
