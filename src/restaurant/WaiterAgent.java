...
public class WaiterAgent extends Agent {
   ...
   //State variables for Waiter
	...
	{NEED_SEATED,READY_TO_ORDER,ORDER_PENDING,ORDER_READY,IS_DONE,NO_ACTION,BILL_READY,NEED_TO_REORDER};
	...
	public Food food; //gui thing
	 //public Bill guiBill=new Bill(restaurant);//gui thing
	//added in V4.1
	public BillFromCustomer bill;
    public boolean updateBill=false;
   ...
   // GuiCashier guiCashier;
	...
	for(MyCustomer c:customers){
		...
		stateChanged();
		print("the number of permits after statechanged is "+this.stateChange.availablePermits());
		return;
	    }
	}
    }
    //Messages added in V4.1
    public void msgHereIsBill(BillFromCustomer bill)
    {
    	for(MyCustomer c:customers){
    		if(bill.getCustomer().equals(c.cmr))
    		{
    			//c.state=CustomerState.BILL_READY;
    			c.updateBill=true;
    			c.bill=bill;
    			stateChanged();
    			return;
    ...
    public void msgRunningOutOfFood(Order o)
    {
    	for(MyCustomer c:customers){
    		if(c.tableNum==o.tableNum)
    		{
    	    	//print("Waiter "+name+" found the customer who need to reorder");
    			c.state=CustomerState.NEED_TO_REORDER;
    			stateChanged();
    			return;
    		}
    	}
    ...
    public void msgUpdateMenu(Menu menu)
    {
    	this.menu=menu;
    	print("Waiter" +this.name+" got the updated menu ");
    ...
    public void msgTooExpensiveIWillLeave(CustomerAgent customer)
    {
    	for(MyCustomer c:customers)
    	{
    		if(c.cmr.equals(customer))
	...
	if(!customers.isEmpty()){
	    ...
	    for(MyCustomer c:customers){
		  ...
		  //Gives the customer a chance to reorder when the food is out of stock
	    for(MyCustomer c:customers){
			if(c.state == CustomerState.NEED_TO_REORDER){
				//print("found a customer who need to reorder in scheduler");				
				askToReorder(c);
			    return true;
			}
		  }
	    //Gives bill to customer if the bill is ready
	    for(MyCustomer c:customers){
//			if(c.state == CustomerState.BILL_READY) {
	    	if(c.updateBill){
	    		c.updateBill=false;
    ...
    private void seatCustomer(MyCustomer customer) {
    	...
    	//print("printing available foods in Waiter: "+name);
    	//menu.printAvailableFood();
    	customer.cmr.msgFollowMeToTable(this, menu);
    	stateChanged();
    }
	...
	customer.state = CustomerState.NO_ACTION;
	cook.msgHereIsAnOrder(new Order(this, customer.tableNum, customer.choice));
	
	print("Giving " + customer.cmr + "'s choice of " + customer.choice + " to cook");
	cashier.msgCustomerOrdered(this, customer.cmr, customer.choice);
	stateChanged();
    ...
    private void giveBillToCustomer(MyCustomer customer) {
    	DoGiveBillToCustomer(customer);//Animation
    	//customer.state = CustomerState.NO_ACTION;
    ...
    private void askToReorder(MyCustomer customer) {
    	DoAskCustomerToReorder(customer); 
    	customer.state=CustomerState.NO_ACTION; 
    	menu.makeItUnavailable(customer.choice); 
    	print("Waiter: "+this.name+" ask the customer "+customer.cmr.getName()+" to reorder.");
    	customer.cmr.msgOutOfChoice(menu); 
    ...
    {
    	print("Waiter: "+this.name+" tell the host that he wants to take a break and wait for permission");
    	host.msgCanITakeABreak(this);
    	stateChanged();
    }
    public void goBackToWork() {
    	print("Waiter: "+this.name+" tell the host that he wants to go back to work");
    	host.msgGoBackToWork(this); 
    	onBreak=false; 
    	stateChanged();
    }
    ...
    private void removeCustomer (MyCustomer customer)
    {
    	for(int i=0;i<customers.size();i++)
    	{
    		...
    		{
    			print("Waiter: "+this.name+" remove customer "+customer.cmr.getName());
    			customers.remove(i);
    			stateChanged();
    			return;
    		}
    	}
    	print("Waiter: "+this.name+" cannot find customer "+customer.cmr.getName()+" when removing");
    }
    // Animation Actions
    ...
    void DoGiveBillToCustomer(MyCustomer customer)
    {
    	print("Waiter: "+this.name+" give bill to customer "+customer.cmr.getName());
//    	Position inFrontOfCashier = new Position (guiCashier.getX()-5,guiCashier.getY());
//    	guiMoveFromCurrentPostionTo(inFrontOfCashier);//in front of grill
//    	guiWaiter.pickUpBill(customer.guiBill);
//    	Position tablePos = new Position(tables[customer.tableNum].getX()-1,
//    					 tables[customer.tableNum].getY()+1);
//    	guiMoveFromCurrentPostionTo(tablePos);
//    	guiWaiter.giveBill(tables[customer.tableNum]);
    }
    void DoAskCustomerToReorder(MyCustomer customer)
    {
    	print("Waiter "+this.name+"Asked Customer at table" + customer.tableNum+" "+customer.cmr.getName()+" to reorder");
    }
	...
	if(customer.food!=null) //if the customer has not ordered yet
